    ...
    def __iter__(self):
        for item in self.data_list:
            ...
            if key in value_list:
                ck = 'checked'  # 在CheckBox中加入checked属性就是默认选中状态
...
def issues(request, project_id):
    if request.method == 'GET':
        ...
        # 分页组件
        issues_object_list = models.Issues.objects.filter(project_id=project_id).filter(**condition)
...
def issues_change(request, project_id, issues_id):
    ...
    if name in ['priority', 'status', 'mode']:
        selected_text = None
        for key, text in field_object.choices:  # 判断合法性防止有人传恶意数据
            if str(key) == value:
                selected_text = text
